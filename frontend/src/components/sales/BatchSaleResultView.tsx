...
export function BatchSaleResultView({ result }: BatchSaleResultViewProps) {
  return (
    <Card className="space-y-4 border-accent/30 bg-accent-soft/30">
      ...
      {result.low_stock_materials.length > 0 && (
        <div className="rounded bg-danger-soft px-3.5 py-3">
          <p className="text-sm font-semibold text-danger">
            {result.low_stock_materials.length}개 원자재가 안전재고 이하로 떨어졌습니다
          </p>
          <p className="mt-1 text-xs text-danger/80">
            {result.low_stock_materials.map((m) => m.material_name).join(", ")} —
            보충이 필요합니다.
          </p>
        </div>
      )}
      ...
      <ul className="divide-y divide-border/60">
        {result.changes.map((change) => (
          ...
          >
            <span className="font-medium">{change.material_name}</span>
